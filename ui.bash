...
function debugging.setup {
  ...
  debuglog="$(mktemp -u -p "${debugdir}" "debuglog-$(basename "${BASH_SOURCE[0]:-$0}")-XXXXX.log")"
...
function logger.__log {
  ...
  color="$2"
  message="$3"
  now="$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N')"
  ln="${ln:-\n}"
...
if [[ "${SHOULD_COLOR}" == "false" ]]; then
  function logger.debug {
    echo "$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N') DEBUG:   $*"
  }

  function logger.info {
    echo "$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N') INFO:    $*"
  }

  function logger.success {
    echo "$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N') SUCCESS: $*"
  }

  function logger.warn {
    echo "$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N') WARNING: $*"
  }

  function logger.error {
    echo "$(TZ='America/Los_Angeles' date '+%H:%M:%S.%3N') ERROR:   $*"
  }
fi
